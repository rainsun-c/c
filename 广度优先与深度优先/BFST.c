...
void BFSTgraph(Graph* graph,int inpos)
{
	//标记
	int visited[MAX] = {0};
	//队列
	...
	if (front < rear)
	{
		...
		while (pMove)
		{
			if (visited[pMove->num] == 0)
			{
				visitVextex(graph->data[pMove->num].ver);//打印
				visited[pMove->num] = 1;//标记打印过的元素
			}
			pMove = pMove->first;
		}
	}
